...
# Hier -q Können die Einstellungen für die Docker-Machine Verändert werden
...
ssh -t -q "$USER@$ip" "PATH=$PATH;echo \"/etc/init.d/services/dhcp stop;ifconfig eth1 10.0.0.$((100+i)) netmask 255.255.255.0 broadcast 10.0.0.255 up\" | docker-machine ssh default sudo tee /var/lib/boot2docker/bootsync.sh > /dev/null"
...
ssh -t -q "$USER@$ip" "PATH=$PATH;docker-machine start default"
